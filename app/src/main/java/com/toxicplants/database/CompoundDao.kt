...
interface CompoundDao {
    ...
    @Query("UPDATE compounds SET pubchemCid = :cid WHERE id = :id")
    suspend fun updatePubchemCid(id: Int, cid: Int)

    /** Cuenta cuántos compuestos ya tienen CID asignado. */
    @Query("SELECT COUNT(*) FROM compounds WHERE pubchemCid != 0")
    suspend fun countWithCid(): Int
