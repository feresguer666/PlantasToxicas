...
class BackupRepository(private val context: Context, private val db: PlantDatabase) {
    ...
    // Bloc de notas de NotesScreen.
    private val plantNotesPrefsName = "plant_notes_prefs"
    private val plantNotesKey = "notes_json"

    // Archivo editable usado por mascotas/niños/ganado/colores/confusables.
    private val plantExtraFileName = "plant_extra.json"
    ...
    suspend fun exportDatabaseToUri(
        ...
        try {
            ...
            val plantNotesJson = loadPlantNotesJson()
            val plantExtraJson = loadPlantExtraJson()
            val poisonousFamilyGenera = db.poisonousFamilyDao().getAllSync()
            ...
            finalOut.use { out ->
                JsonWriter(OutputStreamWriter(out, Charsets.UTF_8)).use { w ->
                    ...
                    // Bloc de notas libre de NotesScreen.
                    w.name("plantNotes").value(plantNotesJson)

                    // Datos extra editables: mascotas, niños, ganado, colores, confusables.
                    w.name("plantExtraJson").value(plantExtraJson)

                    // Nueva pantalla Familias venenosas.
                    w.name("poisonousFamilyGenera").beginArray()
                    for (g in poisonousFamilyGenera) gson.toJson(g, PoisonousFamilyGenusEntity::class.java, w)
                    w.endArray()
    ...
    private fun loadPlantNotesJson(): String {
        val raw = context.getSharedPreferences(plantNotesPrefsName, Context.MODE_PRIVATE)
            .getString(plantNotesKey, "[]")
            ?: "[]"
        return raw.trim().takeIf { it.startsWith("[") && it.endsWith("]") } ?: "[]"
    }

    private fun savePlantNotesJson(rawJson: String) {
        val safeJson = rawJson.trim().takeIf { it.startsWith("[") && it.endsWith("]") } ?: "[]"
        context.getSharedPreferences(plantNotesPrefsName, Context.MODE_PRIVATE)
            .edit()
            .putString(plantNotesKey, safeJson)
            .apply()
    }

    private fun loadPlantExtraJson(): String {
        runCatching { PlantExtraDataSource.loadAll(context) }
        val file = File(context.filesDir, plantExtraFileName)
        val raw = if (file.exists()) file.readText(Charsets.UTF_8) else "[]"
        return raw.trim().takeIf { it.startsWith("[") && it.endsWith("]") } ?: "[]"
    }

    private fun savePlantExtraJson(rawJson: String) {
        val safeJson = rawJson.trim().takeIf { it.startsWith("[") && it.endsWith("]") } ?: "[]"
        File(context.filesDir, plantExtraFileName).writeText(safeJson, Charsets.UTF_8)
    ...
    private suspend fun parseAndRestore(r: JsonReader, progress: ProgressListener?) {
        ...
        while (r.hasNext()) {
            ...
            when (fieldName) {
                ...
                "plantNotes" -> {
                    progress?.onProgress("Restaurando notas…", 0, 1)
                    val rawJson = if (r.peek() == JsonToken.STRING) r.nextString() else null
                    if (rawJson != null) savePlantNotesJson(rawJson)
                    else r.skipValue()
                }

                "plantExtraJson" -> {
                    progress?.onProgress("Restaurando datos extra…", 0, 1)
                    val rawJson = if (r.peek() == JsonToken.STRING) r.nextString() else null
                    if (rawJson != null) savePlantExtraJson(rawJson)
                    else r.skipValue()
                }

                "poisonousFamilyGenera" -> {
                    progress?.onProgress("Restaurando familias venenosas…", 0, 1)
                    val list = ArrayList<PoisonousFamilyGenusEntity>()
                    r.beginArray()
                    while (r.hasNext()) {
                        list += gson.fromJson<PoisonousFamilyGenusEntity>(r, PoisonousFamilyGenusEntity::class.java)
                    }
                    r.endArray()
                    db.poisonousFamilyDao().deleteAll()
                    if (list.isNotEmpty()) db.poisonousFamilyDao().insertAll(list)
                }
