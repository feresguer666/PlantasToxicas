...
object ImageDownloader {
    ...
    // ✅ FORZAR GENERACIÓN DE IMAGEN CON IA (Sáltase la cascada)
    suspend fun forceAiImage(context: Context, plant: PlantEntity): Boolean {
        val aiUrl = AiImageService.generateBotanicalImageUrl(
            plant.scientificName,
            plant.family,
            plant.commonName
        )
        return LocalImageCache.downloadAndSave(context, plant.id, aiUrl)
    }

    // ✅ MÉTODO PÚBLICO PARA RESOLVER IMAGEN DE UNA SOLA PLANTA
    suspend fun resolveImageUrl(context: Context, plant: PlantEntity): String? {
        ...
        if (tryMultipleSources(context, plant)) {
            return "file://${LocalImageCache.getLocalImagePath(context, plant.id)}"
        }
    ...
    private suspend fun tryMultipleSources(context: Context, plant: PlantEntity): Boolean {
        ...
        val sources = listOf(
            ...
            // 9. Generación por IA (Último recurso)
            SourceInfo("IA", 8) { _, _ ->
