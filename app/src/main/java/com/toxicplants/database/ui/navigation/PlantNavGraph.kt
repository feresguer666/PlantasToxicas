...
sealed class Screen(val route: String) {
    ...
    object Intoxication      : Screen("intoxication")
    object ToxicSyndromes    : Screen("toxic_syndromes")
    ...
    object EditCompound      : Screen("edit_compound/{compoundId}") {
        fun createRoute(compoundId: Int) = "edit_compound/$compoundId"
    }
    object Location          : Screen("location/{plantId}") {
    ...
    NavHost(navController = navController, startDestination = Screen.Home.route) {
        ...
        composable(Screen.Home.route) {
            HomeScreen(
                ...
                onNavigateToIntoxication     = { navController.navigate(Screen.Intoxication.route) },
        ...
        composable(
            ...
            PlantDetailScreen(
                ...
                onCompoundClick      = { compound -> navController.navigate(Screen.CompoundDetail.createRoute(compound.id)) }
            )
        }
        ...
        composable(Screen.Search.route) {
            SearchScreen(
                ...
                onBack = { navController.popBackStack() },
                onIntoxicationClick = { navController.navigate(Screen.Intoxication.route) }
            )
        }
        ...
        composable(Screen.SearchBySymptoms.route) {
            SearchBySymptomsScreen(
                ...
                onCompoundClick = { compound: CompoundEntity ->
                    navController.navigate(Screen.CompoundDetail.createRoute(compound.id))
                },
                onBack = { navController.popBackStack() }
            )
        }

        composable(Screen.Intoxication.route) {
            IntoxicationScreen(
                onSymptomsClick = { navController.navigate(Screen.SearchBySymptoms.route) },
                onSyndromesClick = { navController.navigate(Screen.ToxicSyndromes.route) },
                onBack = { navController.popBackStack() }
            )
        }

        composable(Screen.ToxicSyndromes.route) {
        ...
        composable(Screen.Phytochemistry.route) {
            PhytochemistryScreen(
                ...
                onInteractionsClick = { navController.navigate(Screen.CompoundInteractions.route) },
                onCompoundClick     = { c -> navController.navigate(Screen.CompoundDetail.createRoute(c.id)) },
                onBack              = { navController.popBackStack() }
            )
        }

        composable(Screen.CompoundInteractions.route) {
        ...
        composable(Screen.FamilyList.route) {
            FamilyListScreen(
                ...
                onFamilyClick = { family ->
                    navController.navigate(Screen.PlantsByFamily.createRoute(family))
                }
            )
        }

        composable(
            route     = Screen.PlantsByFamily.route,
