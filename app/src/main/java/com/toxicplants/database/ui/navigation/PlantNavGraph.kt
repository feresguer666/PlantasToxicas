...
fun PlantNavGraph(viewModel: PlantViewModel) {
    ...
    NavHost(navController = navController, startDestination = Screen.Home.route) {
        ...
        composable(Screen.Search.route) { SearchScreen(viewModel = viewModel, onPlantClick = { plant: PlantEntity -> viewModel.selectPlant(plant); navController.navigate(Screen.PlantDetail.createRoute(plant.id)) }, onBack = { navController.popBackStack() }) }
        ...
        composable(Screen.BerriesGuide.route) { BerriesGuideScreen(viewModel = viewModel, onBack = { navController.popBackStack() }) }
        ...
        composable(Screen.CameraIdentify.route) {
            CameraIdentifyScreen(viewModel = viewModel, onPlantClick = { plant: PlantEntity -> viewModel.selectPlant(plant); navController.navigate(Screen.PlantDetail.createRoute(plant.id)) }, onNavigateToPlantNetResult = { name, scientificName -> navController.navigate(Screen.PlantNetResult.createRoute(name, scientificName)) }, onBack = { navController.popBackStack() })
        }
        composable(route = Screen.PlantNetResult.route, arguments = listOf(navArgument("name") { type = NavType.StringType }, navArgument("scientificName") { type = NavType.StringType })) { backStackEntry ->
