...
fun BerriesGuideScreen(viewModel: PlantViewModel, onBack: () -> Unit) {
    ...
    Scaffold(
        ...
        if (berries.isEmpty()) {
            Box(modifier = Modifier.fillMaxSize().carbonEffectSubtle().padding(padding), contentAlignment = Alignment.Center) {
                Text("No se encontraron bayas en la base de datos.")
            }
        } else {
            LazyColumn(
                modifier        = Modifier.fillMaxSize().carbonEffectSubtle().padding(padding),
