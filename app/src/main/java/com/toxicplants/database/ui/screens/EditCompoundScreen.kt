...
fun EditCompoundScreen(
    ...
    Scaffold(
        ...
        if (isLoading) {
            Box(modifier = Modifier.fillMaxSize().carbonEffectSubtle().padding(padding), contentAlignment = androidx.compose.ui.Alignment.Center) {
                ...
                Button(
                    onClick = {
                        val newCompound = CompoundEntity(
                            ...
                            pubchemCid = compound?.pubchemCid ?: 0
                        )
                        if (compound == null) {
