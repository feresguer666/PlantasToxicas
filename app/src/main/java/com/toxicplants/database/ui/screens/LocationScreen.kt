    ...
    Scaffold(
        ...
        Column(
            ...
            OutlinedButton(
                ...
                Text(if (showManualInput) "Ocultar coordenadas manuales" else "✏️ Introducir coordenadas manualmente")
            }
            ...
            if (showManualInput) {
                Card(
                    ...
                    Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        Text("Introduce las coordenadas:", fontWeight = FontWeight.Bold, fontSize = 14.sp)
                        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            OutlinedTextField(
                                value = manualLat,
                                onValueChange = { manualLat = it },
                                label = { Text("Latitud") },
                                placeholder = { Text("Ej: 40.4168") },
                        ...
                        Button(
                            onClick = {
                                ...
                                val lng = manualLng.replace(",", ".").toDoubleOrNull()
                                if (lat != null && lng != null && lat in -90.0..90.0 && lng in -180.0..180.0) {
                                    latitude = lat
                                    longitude = lng
                                    statusMessage = "✅ Coordenadas aplicadas"
                                    getAddressFromLocation(context, lat, lng) { address ->
                                        locationName = address
                                    }
                                } else {
                                    statusMessage = "❌ Coordenadas no válidas"
                                }
                            },
                            modifier = Modifier.fillMaxWidth(),
                            colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF1565C0))
                        ) {
                            Text("📌 Aplicar coordenadas")
                        }
                        Text(
                            "Formato: decimal (40.4168, -3.7038)",
                            fontSize = 10.sp,
                            color = Color.Gray
                        )
                    }
